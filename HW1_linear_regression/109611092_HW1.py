...
class LinearRegression:
    ...
    def closed_form_fit(self, X, y):
        ...
        X_tran = X_intercept.T
        X_tran_X = np.dot(X_tran, X_intercept)
        X_tran_Y = np.dot(X_tran, y)

        self.closed_form_weights = np.dot(np.linalg.inv(X_tran_X), X_tran_Y)[1:]
        self.closed_form_intercept = np.dot(np.linalg.inv(X_tran_X), X_tran_Y)[0]
        pass
    ...
    def gradient_descent_fit(self, X, y, lr, epochs):
        ...
        for epoch in range(epochs):
            ...
            for i in range(sample_num): 
                ...
                gradient = (-2) * x_i * loss
                theta -= lr * gradient
            self.mse_loss.append(self.get_mse_loss(np.dot(X_intercept, theta), y))
    ...
    def get_mse_loss(self, prediction, ground_truth):
        # Return the value.
        return np.sum((prediction - ground_truth)**2)/ground_truth.shape[0]
        pass
    ...
    def closed_form_predict(self, X):
        ...
        prediction = np.dot(X, self.closed_form_weights) + self.closed_form_intercept
        return prediction
        pass
    ...
    def gradient_descent_predict(self, X):
        # Return the prediction.
        prediction = np.dot(X, self.gradient_descent_weights) + self.gradient_descent_intercept
        return prediction
        pass
    ...
    def plot_learning_curve(self):
        ...
        plt.plot(range(1, len(self.mse_loss) + 1), self.mse_loss, marker='o', linestyle='-', label = 'Training Loss')
...
if __name__ == "__main__":
    ...
    #LR.gradient_descent_fit(train_x, train_y, lr=0.0003, epochs=500)
    LR.gradient_descent_fit(train_x, train_y, lr=0.0008, epochs=400) 
